...
public interface EFGRepository extends CrudRepository<EFG, Integer> {
	...
	@Query(value = "SELECT e FROM EFG e WHERE e.créateur.idCanal=?1")	
	public List<EFG> findByCanal(int idCanal);
	
	@Query(value ="SELECT COUNT(m) FROM MembreCanal m WHERE m.idCanal=?1")
	public int nbreMembreCanal(int idCanal);
